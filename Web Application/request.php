...
<head>
	<meta charset="UTF-8">
	<title>AllActive - Requests</title>
...
<div id="container">
	...
	<?php if( !empty($user) ): ?> <!-- User is logged in -->
		...
		<div id="mainalt">
			<br /><br /><h3>Requests</h3>
			<?php 
				$current_id = $_SESSION['user_id'];
				$request_query = $conn->prepare("SELECT sender FROM friend_request WHERE receiver='$current_id'");
				$request_query->execute();
				
				if ($request_query->rowCount() > 0) {
					while ($run_request = $request_query->fetch(PDO::FETCH_ASSOC)) {
						$sender = $run_request['sender'];
						$sender_username = getUser($sender, 'username');
						echo "<a href='profile.php?user=$sender' class='list' style='display:block'>$sender_username</a>";
					}
				} else {
